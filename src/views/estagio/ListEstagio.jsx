...
export default function ListEstagios() {
  ...
  useEffect(() => {
    if (getEstagios) {
      getEstagios.forEach(async (estagio) => {
        const link = await pdf({ id: estagio.pdf });
        setLinks((prevLinks) => ({
          ...prevLinks,
          [estagio._id]: link,
        }));
  ...
  function filtrarEstagios(estagios) {
    return estagios.filter((e) => {
      const matchesDescricao = e.descricao?.toLowerCase().includes(descricao.toLowerCase());
      const matchesEstudante = idEstudante ? e.estudante_nome?.toLowerCase().includes(idEstudante.toLowerCase()) : true;
      ...
      <div style={{ marginTop: "3%" }}>
        <Container textAlign="justified">
          ...
          <div style={{ marginTop: "4%" }}>
            <Menu compact>
              <Menu.Item name="menuFiltro" active={menuFiltro === true} onClick={handleMenuFiltro}>
                <Icon name="filter" />
                Filtrar
              </Menu.Item>
            </Menu>
                  ...
                  filtrarEstagios(getEstagios).map((e) => (
                    <Table.Row key={e.id}>
                      ...
                      <Table.Cell textAlign="center">
                        <Button inverted circular color="green" title="Editar">
                          <Link to="/form-estagios" state={e} style={{ color: "green" }}>
                            <Icon name="edit" />
                          </Link>
                        </Button>{" "}
                        <Button inverted circular icon="trash" color="red" title="Remover" onClick={() => confirmaRemover(e._id)} />
                      </Table.Cell>
                    </Table.Row>
                  ))}
              </Table.Body>
